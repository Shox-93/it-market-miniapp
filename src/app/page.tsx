...
export default function ITMarketMiniAppUI() {
  ...
  const [activeSection, setActiveSection] = useState<keyof typeof sampleItems>("vacancies");
  ...
  const filteredItems = useMemo(() => {
    ...
    return list.filter(
      (item) => item.title.toLowerCase().includes(q) || item.meta.toLowerCase().includes(q)
    );
  }, [activeSection, query]);
  ...
  return (
    <div className="min-h-screen bg-[#050b25] text-white">
      <div className="mx-auto flex min-h-screen w-full max-w-md flex-col px-4 pb-28 pt-5">
        ...
        >
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-cyan-300/80">IT-MARKET</p>
            <h1 className="mt-1 text-2xl font-bold">Пользовательская платформа</h1>
          </div>
            ...
            <CardContent className="p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xl font-bold">Shaxzod Sobitaliev</p>
                  ...
                  <div className="mt-4 flex items-center gap-3 rounded-[24px] bg-[#091235] px-4 py-4 ring-1 ring-white/5">
                    ...
                    <Button size="icon" className="h-12 w-12 rounded-full bg-cyan-400 text-slate-950 hover:bg-cyan-300">
        ...
        <div className="mt-5 grid grid-cols-2 gap-4">
          <motion.button
            ...
            className="group col-span-1 row-span-2 rounded-[28px] bg-gradient-to-br from-[#171f54] to-[#10173d] p-5 text-left ring-1 ring-white/8"
          >
            <p className="text-[2rem] font-extrabold leading-none">Каталог</p>
            <p className="mt-2 text-sm text-white/60">Вакансии, компании и специалисты</p>
            <div className="mt-8 flex justify-center">
              <div className="rounded-[28px] bg-gradient-to-br from-indigo-500/70 to-cyan-400/40 p-8 shadow-xl shadow-cyan-500/10 transition-transform group-hover:scale-105">
          ...
          <motion.button
            ...
            className="rounded-[28px] bg-gradient-to-br from-amber-500/50 to-rose-500/40 p-5 text-left ring-1 ring-white/8"
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-3xl font-bold">Stars</p>
                <p className="mt-1 text-sm text-white/70">Избранное и сохранённое</p>
              </div>
          ...
          <motion.button
            ...
            className="rounded-[28px] bg-gradient-to-br from-blue-700/60 to-fuchsia-500/35 p-5 text-left ring-1 ring-white/8"
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-3xl font-bold">Premium</p>
                <p className="mt-1 text-sm text-white/70">Заказы и новые проекты</p>
              </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-white/60">Рейтинг платформы</p>
                  </div>
                  <div className="rounded-full bg-white/80 p-2 text-slate-900">
        ...
        <div className="mt-6 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <button className="rounded-[22px] bg-gradient-to-r from-pink-600 to-orange-400 px-4 py-4 text-left shadow-lg shadow-pink-500/20">
              <p className="text-2xl font-bold">Instagram</p>
              <p className="text-sm text-white/85">Скидки и ролики</p>
            </button>
            <button className="rounded-[22px] bg-gradient-to-r from-sky-500 to-indigo-500 px-4 py-4 text-left shadow-lg shadow-sky-500/20">
        ...
        <div className="mt-7 rounded-[28px] bg-[#0d1438] p-4 ring-1 ring-white/8">
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-white/60">Раздел: {activeSection === "favorites" ? "Избранное" : activeSection === "orders" ? "Заказы" : activeSection === "companies" ? "Компании" : activeSection === "specialists" ? "Специалисты" : "Вакансии"}</p>
            </div>
            ...
            {sections.map((section) => {
              ...
              return (
                <motion.button
                  key={section.id}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => setActiveSection(section.id as keyof typeof sampleItems)}
                  className={`rounded-[24px] bg-gradient-to-br ${section.gradient} p-4 text-left ring-1 transition ${
                    active ? "ring-cyan-300/60 shadow-xl" : "ring-white/8"
                  } ${section.accent}`}
                >
                  <div className="flex items-center justify-between">
                    <Icon className="h-8 w-8 text-white" />
                    <span className="rounded-full bg-white/15 px-2.5 py-1 text-xs font-semibold">{section.badge}</span>
                  </div>
                  <p className="mt-6 text-xl font-bold">{section.title}</p>
                  <p className="mt-1 text-sm text-white/65">{section.subtitle}</p>
                </motion.button>
              );
            })}
          </div>
            ...
            {filteredItems.map((item, index) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Button className="rounded-xl bg-cyan-400 text-slate-950 hover:bg-cyan-300">Открыть</Button>
                </div>
              </motion.div>
            ))}
